...
=== "Shopify"
    ...
    In this tutorial you will learn how to use [Jump logic](#jump-logic), [Skip logic](#skip-logic) and [Display logic](#display-logic). Each one shows or hides questions and content, in the quiz and on the results page.
...
=== "Shopify"
    ...
    **Objective**: Show custom content based on customer answers, using [Jump logic](#jump-logic), [Skip logic](#skip-logic) or [Display logic](#display-logic) on the results page.



=== "Shopify (Legacy)"

    In this tutorial you will learn how to use [conditional logic](/how-to-guides/use-conditional-logic/) to give personalized advice.
    ...
    **Objective**: Show these texts using [Jump Logic](#jump-logic), [Skip Logic](#skip-logic) or [Display Logic](#display-logic) on the results page.
...
=== "Shopify"
    ...
    6. **Adjust Default Destinations for Each Statement.** Open each statement slide in turn. In the Jump logic tab, set `Default destination` to the next question, so the other statements are skipped.
    ...
    1. Set up Multiple Results pages in your quiz.
        ...
        !!! tip
            To learn how to create multiple results pages, see [how to set up multiple results pages](/how-to-guides/set-multiple-result-pages/).
...
=== "Shopify (Legacy)"
    ...
    1. To display text advice in the quiz, add a `Statement question`.
    2. Make it longer by adding a description.
    3. One statement will be needed for each type of skin advice.

    To make only one block appear, instead of all of them one after the other, use [Jump Logic](/how-to-guides/use-jump-logic/).

    1. Open the [Conditional Logic](/reference/quiz-builder/conditional-logic/) tab and select a question.
    2. Add the first [Jump logic](/reference/quiz-builder/conditional-logic/#jump-logic) condition:
    ...
    5. Add conditions for the *Combination* and *Normal* skin.
    6. Add [Jump logic](/reference/quiz-builder/conditional-logic/#jump-logic) to each statement as well, so the customer does not see all four.
    7. Click on a `statement` question.
    8. In the `Always jump to…` section, select the question or page that should follow it.
    9. Do the same for the other three statements.
    10. Click the top-right `Publish` button to update the preview and the live quiz, then test it with `Preview`.
        - First, select `Dry skin`.
        - Now go back and select `Oily skin`.
    ...
    !!! tip "Jump Logic - other use cases"
        ...
        [Jump Logic](/how-to-guides/use-jump-logic/) can also:
...
=== "Shopify"
    ...
    2. Add multiple [`Statement` slides](/reference/quiz-builder/questions/#question-types) describing the specific skin type and its challenges. For example:
    ...
    3. To add [Skip logic](/how-to-guides/use-skip-logic/), go to the [Conditional logic](/reference/quiz-builder/conditional-logic/) tab and select the first statement question that should be skipped.
    ...
    6. Add the same kind of rule to the other statements, so only the relevant one appears.
    ...
    1. In the [Quiz builder](/reference/quiz-builder/), add a multiple-choice question that **accepts multiple selections**. Turn that on in the [Multiple Choice Block settings](/reference/quiz-builder/questions/#multiple-choice). Create several follow-up questions that match the choices in the initial question.
...
=== "Shopify (Legacy)"
    ...
    5. Test the quiz with the `Preview` button.
...
=== "Shopify"
    ...
    1. **Create Quiz**: Open the [Quiz builder](/reference/quiz-builder/) and add `Multiple-choice questions` about the needs of the customer. For a quiz that works out a skin type, ask about age and skin condition.
    ...
    5. Click the `Search or create variable` search bar and type a variable name, such as `dry skin` or `variable1`.
    6. A dropdown then offers `Create a new variable "xxx"`. Click it to add the variable.
    ...
    5. Publish the changes with the top-right `Save` button to update the preview/live quiz and test the setup. Preview the quiz by clicking `Preview` button and select different skin types to ensure only the relevant section is displayed.
        ...
        !!! warning
            ...
            Too many conditions make the logic hard to follow. At that point, build the quiz [Based on winning variable](/tutorials/conditional-logic/#based-on-winning-variable) or [Based on total score](/tutorials/conditional-logic/#based-on-total-score) instead.
    ...
    2. Assign variables to choices. Each choice is associated with a [custom variable and a score](#add-scores-or-variables-to-choices) via the [`Choices Settings`](/reference/quiz-builder/questions/#choice-settings) section.


        !!! tip

            To learn how to add scores or variables to choices, see [Add scores or variables to choices](#add-scores-or-variables-to-choices).
    ...
    4. Make sure each section contains a [product block](/reference/quiz-builder/results-page/#product-product-variants-collections) to recommend products for that skin type.
    ...
    2. Assign variables to choices. Each choice is associated with a [custom variable and a score](#add-scores-or-variables-to-choices) via the [`Choices Settings`](/reference/quiz-builder/questions/#choice-settings) section.
        ...
        !!! example "Example: Assigning scores to choices"
...
=== "Shopify (Legacy)"
    ...
    1. Add four `Content blocks` with the skin type advice to your [Results Page](/reference/quiz-builder/results-page/).
    2. You can edit the block text with [Markdown language](/how-to-guides/use-markdown/).
    3. To make only one block appear, instead of all of them, add [Display logic](/reference/quiz-builder/results-page/#display-logic-display-section-logic).
    4. To do that, activate it in the lower right corner with the `...` button.
    5. Add the first logic condition.
    ...
    6. Add similar rules to the other content blocks.
    7. Publish the changes with the top-right `Publish` button, then test the quiz again with `Preview`.

    Display logic now shows and hides content on the Results page.

    !!! tip "Display Logic - other use cases"

        [Display Logic](/how-to-guides/use-display-logic/) can also:
