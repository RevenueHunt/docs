...
=== "Shopify"
    ...
    Plan enough questions for one skin type to pull clearly ahead. With five types and one point per answer, a three-question quiz ties often. Six to eight questions give each type room to win.

    !!! example "A sample matrix of skin types and recommendations"
    ...
    5. **Repeat the process for each choice in that question and move on to the next question in your quiz.**
        ...
        !!! example "Sample scores and variables"
    ...
    5. **In the [Product block settings](/reference/quiz-builder/results-page/#product-product-variants-collections), set the `Recommendation system` to `Fixed Recommendations`.** The section is already matched to a result. You pick its products yourself, rather than letting the upvote system choose.
    ...
    3. **Set the condition to IF `the variable with the highest score` `IS` `dry_skin`, THEN this section is VISIBLE.** Set the default visibility to `HIDDEN`, so the section stays hidden until its variable wins.

    4. **Repeat for each skin type section, using that section's own variable.**
    ...
    !!! note "Two variables can tie"

        `the variable with the highest score` does not break a tie for you. If two skin types finish level, decide what should happen and write it into the conditions.

        [Display logic](/reference/quiz-builder/results-page/#display-logic-display-section-logic) also reads a variable's score directly, with `is equal to`, `is greater than`, `is less than` and the or-equal-to forms. Use those to set a threshold, such as showing the dry skin section only when `dry_skin` `is greater than or equal to` `4`.
...
=== "Shopify (Legacy)"
    ...
    The scoring system is not available in the legacy version of the RevenueHunt app for Shopify. For this reason building a personality-type quiz or a scoring quiz like this one is not possible with the off-the-shelf solution.

    Your developer can implement a custom scoring system with JavaScript on the Results Page instead.

    !!! tip
        To recommend products by how many choices the customer picked, see [how to recommend products based on the number of choices](/how-to-guides/recommend-products-based-on-number-of-user-choices/). That guide covers this version.
...
=== "WooCommerce"
    ...
    The scoring system is not available in the RevenueHunt app for WooCommerce. For this reason building a personality-type quiz or a scoring quiz like this one is not possible with the off-the-shelf solution.
...
=== "Magento"
    ...
    The scoring system is not available in the RevenueHunt app for Magento. For this reason building a personality-type quiz or a scoring quiz like this one is not possible with the off-the-shelf solution.
...
=== "BigCommerce"
    ...
    The scoring system is not available in the RevenueHunt app for BigCommerce. For this reason building a personality-type quiz or a scoring quiz like this one is not possible with the off-the-shelf solution.
...
=== "Standalone"
    ...
    The scoring system is not available in the Standalone version of the RevenueHunt app. For this reason building a personality-type quiz or a scoring quiz like this one is not possible with the off-the-shelf solution.
...
---
This article explains how to build a quiz that finds the customer's skin type and recommends the right products.
