...
=== "Shopify"
    ...
    !!! tip "Tutorial"
        For a worked example, follow the tutorial [Sending Follow-up Emails with Klaviyo](/tutorials/follow-up-emails-klaviyo/).
...
=== "Shopify (Legacy)"
    ...
    1. To find your Public Key, log in to your Klaviyo account. In account `Settings` open the `API Keys` tab and copy the public API Key. You can get your Klaviyo Public API Key from the [Klaviyo API Keys tab](https://www.klaviyo.com/account#api-keys-tab).
...
=== "Shopify"
    ...
    4. **Create an Email Flow**: create a flow that starts when someone is added to the segment from the previous step. This is the hardest part, because the emails have to be built by hand in Klaviyo.
        ...
        **Trigger the Flow**
        ...
        1. To build a flow that reaches only customers who finished the quiz, open the `Flows` tab in Klaviyo.
    ...
    5. **Edit the Email**: In the next steps, you should edit the email template.
...
=== "Shopify"
    ...
    4. Under the `Klaviyo list` dropdown, select the list to add each customer to.
...
=== "Shopify (Legacy)"
    ...
    1. To do that you need a **Private API Key**.
...
=== "Shopify"
    ...
    A Klaviyo email template is built from HTML, CSS and the [Django templating](https://docs.djangoproject.com/en/1.8/ref/templates/builtins/) system, so you need a developer to restyle the supplied template to your brand.
    ...
    **Custom Properties in Klaviyo**
    ...
    ??? warning "Overwriting or Appending Custom Properties"
        ...
        Some properties arrive as an array: `ANSWER_BY_BLOCK`, `CHOICE`, `RESPONSE_ID`, `RESULT_REF`, `RESULT_SECTIONS` and `TAGS`. Each quiz completion **overwrites** the previous values.
...
=== "Shopify"
    ...
    1. To show the recommended products, use this code from the Klaviyo template in the [`Integrations`](/reference/quiz-builder/connect-integrations/) tab:
        ...
        ??? example "Example: code from the Integrations tab"
...
Klaviyo can pull the product information straight from Shopify, given the product id. Rather than the `description` or `image_url` the quiz sends, pass the product's `origin_id` and let Klaviyo read the rest from Shopify. See the Klaviyo [Overview of the Catalog Lookup Tag](https://help.klaviyo.com/hc/en-us/articles/360004785571-Overview-of-the-Catalog-Lookup-Tag).
